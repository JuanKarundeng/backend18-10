...
export const createMasukData = async (req, res) => {
  ...
  try {
    await MasukData.create({
      ...
      validasi: validasi,
      harga_diharapkan: formattedHargaAsli,
      keterangan: status,
      foto_nota: file1,
      foto_km_awal: file2,
      foto_km_akhir: file3,
...
export const updateHargaDiharapkan = async (req, res) => {
  const { harga_disetujui } = req.body;
  try {
    ...
    res.status(400).json({ msg: "Gagal DiUpdate" });
  }
};
